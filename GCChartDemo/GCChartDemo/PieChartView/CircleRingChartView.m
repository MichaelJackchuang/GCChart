...
- (void)resetCircleRing {
    ...
    [pieView addSubview:lblCenterTitle];
    
    UILabel *lblCenterTotal = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(lblCenterTitle.frame), 80, 21)];
    lblCenterTotal.center = CGPointMake(pieView.bounds.size.width / 2, lblCenterTotal.center.y);
    lblCenterTotal.font = [UIFont systemFontOfSize:16];
    lblCenterTotal.textColor = [UIColor blackColor];
    lblCenterTotal.text = [NSString stringWithFormat:@"%.f",total];
    lblCenterTotal.textAlignment = NSTextAlignmentCenter;
    [pieView addSubview:lblCenterTotal];
